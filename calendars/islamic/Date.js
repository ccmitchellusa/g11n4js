define(["dojo/_base/lang", "dojo/_base/declare", "../gregorian"], function(lang, declare, gregorian){
	...
	toGregorian:function(){
		...
		var tjd = (this._GREGORIAN_EPOCH - 1) + (365 * (year - 1)) + Math.floor((year - 1) / 4)
				...
				+ ( (gregorian.isLeapYear(new Date(year,3,1)) ? -1 : -2)) + 1);
			
		var leapadj = ((wjd < tjd ) ? 0 : (gregorian.isLeapYear(new Date(year,3,1)) ? 1 : 2));
		...
		var tjd2 = (this._GREGORIAN_EPOCH - 1) + (365 * (year - 1))
					...
					+ ((month <= 2) ? 0 : (gregorian.isLeapYear(new Date(year,month,1)) ? -1 : -2)) + 1);
	...
	fromGregorian:function(/*Date*/gdate){
		...
		var julianDay = (this._GREGORIAN_EPOCH - 1) + (365 * (gYear - 1)) + Math.floor((gYear - 1) / 4)
					...
					+ (((gMonth+1) <= 2) ? 0 : (gregorian.isLeapYear(date) ? -1 : -2)) + gDay);
