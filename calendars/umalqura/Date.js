 ...
 function(lang, declare, gregorian, IDate){
    ...
    toGregorian: function(){
        ...
        if(hYear >= this._hijriBegin && hYear <= this._hijriEnd){
            ...
            gdate = gregorian.add(gregRef, "day", days);
        }
    ...
    fromGregorian: function(/*Date*/ gdate){
        ...
        //if(gregorian.compare(date, gregorianRef[0]) >= 0 && gregorian.compare(date, gregorianRef[16]) <= 0){
		if(gregorian.compare(date, gregorianRef[0]) >= 0 && gregorian.compare(date, gregorianLastRef) <= 0){
		var diff;
			if(gregorian.compare(date, gregorianRef[16]) <= 0){
				var count = 0;
				var pos = 0;
				var isRef=0;
				for(count = 0; count < gregorianRef.length; count++){
					//if(dojo.date.compare(date, gregorianRef[count], "date") == 0){
					if(gregorian.compare(date, gregorianRef[count], "date") == 0){
						pos = count;
						isRef=1;
						break;
					}
				...
				if(isRef==1){
					...
					diff = gregorian.difference(gregorianRef[pos], date, "day");
					pos = pos * 5;
					for(i = pos; i < pos + 5; i++){
						for(j = 0; j <= 11; j++){
	...
	//			if((gregorian.compare(date, gregorianRef[16]) > 0)&& (gregorian.compare(date,gregorianLastRef)<=0)){
				//Date is in year 1480
				diff=gregorian.difference(gregorianRef[16],date,"day");
				var x=gregorian.difference(new Date(2057, 6, 3, 0, 0, 0, 0),new Date(2057, 6, 1, 0, 0, 0, 0),"date");
				//alert (x);
				for( j=0;j<=11;j++){
