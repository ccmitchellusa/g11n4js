...
doh.register("g11n4js.calendars.tests.hebrew.Date",
	[
		...
		},
		{
			name: "compare",
			runTest: function(t){
				var dateHebrew = new hDate(5769, 5, 16);
				var dateHebrew1 = new hDate(5758,  10,  25);
				t.is(1, gregorian.compare(dateHebrew.toGregorian(), dateHebrew1.toGregorian()));
				t.is(-1, gregorian.compare(dateHebrew1.toGregorian(), dateHebrew.toGregorian()));
			}
		},
		{
			...
			var TISHRI = 0 , HESHVAN = 1, KISLEV = 2, TEVET = 3, SHVAT = 4 , ADARI = 5, ADAR = 6, NISAN= 7 , IYAR= 8 , SIVAN= 9 , TAMMUZ = 10, AV = 11 , ELUL = 12;
				...
				arr.forEach(dateTable, function(d, i){
					dateHebrew = new hDate(d[3], d[4], d[5]);
					dateGregorian = dateHebrew.toGregorian();
					t.is(0, gregorian.compare(new Date(d[0], d[1]-1, d[2]), dateGregorian, "date"));
				});
				
				//fromGregorian
				arr.forEach(dateTable, function(d, i){
					dateGregorian = new  Date(d[0], d[1]-1, d[2]);
					dateHebrew = new hDate(dateGregorian);
					testHebrew = new hDate(d[3], d[4], d[5]);
					t.is(0, gregorian.compare(testHebrew.toGregorian() , dateHebrew.toGregorian(),  "date"));
				});
				
				//test for invalid date, toGregorian and fromGregorian should return invalid date
				var invDate = new Date("");
				var hebrewDate = new hDate(invDate);
				var hebrInvDate = new hDate("");
				t.is(true, isNaN(hebrewDate));
				t.is(true, isNaN(hebrewDate.toGregorian()));
		...
		},
		{
			...
			runTest: function(t){
				 var dateTable = [
					...
					arr.forEach(dateTable, function(d, i){
					var date = new  hDate(d[0], d[1], d[2]);
		...
		},
		{
			...
			runTest: function(t){
				...
				arr.forEach(dateTable, function(d, i){
					var date = new  hDate(d[0], d[1], 1);
					t.is(d[2], 	hebrew.getDaysInMonth(date));
				});
			}
		},
		{
		...
		//Hebrew month names
				...
				var start = [
					...
					arr.forEach(start, function(s, i){
						dateHebStart = new hDate( s[0], s[1], 1);
						dateHebRes = hebrew.add(dateHebStart, "month", add[i]);
					...
					//month difference
					arr.forEach(start, function(s, i){
						dateHebRes = new hDate( s[2], s[3], 1);
						dateHebStart =  new hDate( s[0], s[1], 1);
						t.is(add[i], hebrew.difference(dateHebStart , dateHebRes, "month"));
					});
					...
					//test add
					arr.forEach( fields, function(f, i){
						dateHebStart =  new hDate( f[0], f[1], f[2]);
						dateHebEnd = hebrew.add(dateHebStart, f[3], f[4]);
						dateHebRes = new hDate( f[5], f[6], f[7]);
						t.is(0, gregorian.compare (dateHebRes.toGregorian(), dateHebEnd.toGregorian()));
						if(f[3] == "week"){
					...
					arr.forEach( fields, function(f, i){
						dateHebStart =  new hDate( f[0], f[1], f[2]);
						dateHebRes = new hDate( f[5], f[6], f[7]);
						res = hebrew.difference(dateHebStart, dateHebRes, f[3]);
						t.is(f[4], res);
					});
					
					// check that setMonth (getMonth + d) is not the same as add month (d), due to adar
					dateHebStart = new hDate( 5769, KISLEV, 1);
					var dateHebAdd =  hebrew.add(dateHebStart, "month", 4);
		...
		},
		{
			...
			runTest: function(t){
				var dateHebrew = new hDate(5769, 4, 16);
				var dateHebrewLeap = new hDate(5768, 5, 16);
			...
			//	var m =  ["TISHRI " , "HESHVAN", "KISLEV", "TEVET", "SHVAT" , "ADARI", "ADAR", "NISAN" , "IYAR" , "SIVAN" , "TAMMUZ", "AV" , "ELUL"];
				...
				arr.forEach( amouts, function(amount, i){
					dateHebrewAdd = hebrew.add(dateHebrew, "month",  amount);
					dateHebrewAddLeap = hebrew.add(dateHebrewLeap, "month",  amount);
					t.is(hebrew.difference(dateHebrew, dateHebrewAdd, "month"), amount);
					t.is(amount, hebrew.difference(dateHebrewLeap, dateHebrewAddLeap, "month"));
												
					dateHebrewAdd= hebrew.add(dateHebrew, "year", amount);
					t.is(amount, hebrew.difference(dateHebrew, dateHebrewAdd, "year"));
					t.is(amount, hebrew.difference(dateHebrewLeap, hebrew.add(dateHebrewLeap, "year", amount), "year"));
					
					dateHebrewAdd= hebrew.add(dateHebrew, "week",  amount);
					t.is(amount,  hebrew.difference(dateHebrew, dateHebrewAdd, "week"));
					t.is(amount,  hebrew.difference(dateHebrewLeap, hebrew.add(dateHebrewLeap, "week", amount), "week"));
									
					dateHebrewAdd= hebrew.add(dateHebrew, "weekday", amount);
					t.is(amount,  hebrew.difference(dateHebrew, dateHebrewAdd, "weekday"));
					dateHebrewAddLeap = hebrew.add(dateHebrewLeap, "weekday", amount);
					t.is(amount,  hebrew.difference(dateHebrewLeap, hebrew.add(dateHebrewLeap, "weekday", amount), "weekday"));
					
					dateHebrewAdd= hebrew.add(dateHebrew, "day", amount)
					t.is(amount,  hebrew.difference(dateHebrew, dateHebrewAdd, "day"));
					t.is(amount, hebrew.difference(dateHebrewLeap, hebrew.add(dateHebrewLeap, "day", amount), "day"));
										
					dateHebrewAdd= hebrew.add(dateHebrew, "hour", amount);
					t.is(amount,  hebrew.difference(dateHebrew, dateHebrewAdd, "hour"));
					t.is(amount,  hebrew.difference(dateHebrewLeap, hebrew.add(dateHebrewLeap, "hour", amount), "hour"));
					
					dateHebrewAdd= hebrew.add(dateHebrew, "minute", amount);
					t.is(amount,  hebrew.difference(dateHebrew, dateHebrewAdd, "minute"));
					t.is(amount,  hebrew.difference(dateHebrewLeap, hebrew.add(dateHebrewLeap, "minute", amount), "minute"));
					
					dateHebrewAdd= hebrew.add(dateHebrew, "second", amount);
					t.is(amount,  hebrew.difference(dateHebrew, dateHebrewAdd, "second"));
					t.is(amount,  hebrew.difference(dateHebrewLeap, hebrew.add(dateHebrewLeap, "second", amount), "second"));
					
					dateHebrewAdd= hebrew.add(dateHebrew, "millisecond", amount);
					t.is(amount,  hebrew.difference(dateHebrew, dateHebrewAdd, "millisecond"));
					t.is(amount,  hebrew.difference(dateHebrewLeap, hebrew.add(dateHebrewLeap, "millisecond", amount), "millisecond"));
				 });
				 
				var dateHebrewDiff = new hDate(5769, 4, 17);
				t.is(1, hebrew.difference(dateHebrew, dateHebrewDiff));
			}
		},
		{
		...
		// ELUL - 12,  ADAR -6, ADARI -5
				var dateHebrew = new hDate(5765, 1, 1);
		...
		},
		{
			...
			runTest: function(t){
				...
				arr.forEach(dates, function(date, i){
					dateHebrew = new hDate(date[0], date[1], date[2]);
					...
					arr.forEach(options, function(opt, i){
						str= hLocale.format(dateHebrew, opt);
						var option = "{" +opt+", locale:'he'}";
						dateHebrew1 = hLocale.parse(str, opt);
						t.is(0, gregorian.compare(dateHebrew.toGregorian(), dateHebrew1.toGregorian(), 'date'));
					});
					...
					arr.forEach( pattern, function(pat, i){
						...
						str= hLocale.format(dateHebrew, options);
						dateHebrew1 = hLocale.parse(str, options);
						t.is(0, gregorian.compare(dateHebrew.toGregorian(), dateHebrew1.toGregorian(), 'date'));
					});
				});
						
				dateHebrew = new hDate(5769, 6, 3, 15, 3, 59);
				pattern = 'HH$mm$ss';
				options = {timePattern:pattern, selector:'time'};
				str= hLocale.format(dateHebrew, options);
				dateHebrew1 = hLocale.parse(str, options);
				var gregDate = dojo.date.locale.parse(str, options);
				t.is(0, gregorian.compare(gregDate, dateHebrew1.toGregorian(), 'time'));
		...
		},
		{
			...
			runTest: function(t){												
				...
				arr.forEach(hebrewDates, function(date, i){
					dateHebrew = new hDate(date[0], date[1], date[2], date[3], date[4]);
					date2 = new Date(dates[i][0], dates[i][1], dates[i][2], dates[i][3], dates[i][4]);			
		
					var newHebrewDate = hebrew.add(dateHebrew, "millisecond",  1200);
					...
					newHebrewDate = hebrew.add(dateHebrew, "millisecond",  12022);
					...
					newHebrewDate = hebrew.add(dateHebrew, "millisecond",  120422);
					...
					newHebrewDate = hebrew.add(dateHebrew, "millisecond",  1204422);
